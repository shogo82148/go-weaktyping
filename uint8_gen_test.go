...
func TestMarshalUint8_Generated(t *testing.T) {
	testcases := []struct {
		in  interface{}
		out string
	}{
...
func TestUnmarshalUint8_Generated(t *testing.T) {
	testcases := []struct {
		in  string
		ptr interface{}
		out interface{}
		...
		{`{"foo":""}`, new(map[string]Uint8), &map[string]Uint8{"foo": 0}},
		{`{"foo":""}`, new(map[string]*Uint8), &map[string]*Uint8{"foo": PtrUint8(0)}},
